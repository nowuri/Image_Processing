...
void CMFCApplication1Doc::OnHistogram()
{
	...
	for (i = 0; i < 256; i++) { //색의 개수 0~255 총 256개 (가로)
		for (j = 0; j < m_Scale_HIST[i]; j++) //0부터 각 밝기 값의 개수까지(세로) ex. 밝기 0의 픽셀이 5개면 0부터 4까지
			//색칠할 영역 표기
...
void CMFCApplication1Doc::OnHistoEqual()
{
	...
	for (i = 0; i < m_size; i++) {
		Temp = m_InputImage[i]; //InputImage[i]의 값(밝기 값)을 Temp에 저장
...
{
	// TODO: 여기에 구현 코드 추가.
	// D가 붙은 것은 원하는 영상 관련 변수, 없는 것은 입력 영상 관련 변수
	...
	//룩업 테이블을 이용한 명세화
	for (;;) {
		//명도값과 가장 가까운 평활화 값을 찾고 해당 평활화 값에 해당하는 명도 값을 룩업테이블로 넣어준다.
		// 이 과정을 누적 히스토그램[bottom]~ 누적히스토그램[top] 사이에 해당하는 i 값을 for loop을 돌림
		// for loop은 m_Table[i] 값을 top으로 설정하는 코드
		for (i = m_Sum_Of_ScHIST[bottom]; i <= m_Sum_Of_ScHIST[top]; i++) {
			m_TABLE[i] = top;
		}

		//top과 bottom의 범위를 1씩 줄여서 과정을 반복함
		top = bottom;
		bottom = bottom - 1;

		//픽셀의 최소 값 = 0 => bottom이 0보다 작을 때 for loop 빠져 나옴
		if (bottom < -1)
			break;
	}
	...
	for (i = 0; i < m_size; i++) {
		...
		//DADD 값을 룩업테이블에 넣어서 나온 값을 결과 값에 넣음
...
// 영역처리 엠보싱 필터
void CMFCApplication1Doc::OnEmbossing()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, sum = 0;
	...
	//평균값이 없으면 어두운 화면이 나옴
	for (i = 0; i < m_Re_size; i++) {
		sum += (int)m_InputImage[i];
	}
	int avg = (int)(sum / m_Re_size);

	for (i = 0; i < m_Re_height; i++) {
		for (j = 0; j < m_Re_width; j++) {
			int value = m_tempImage[i][j] + avg;
			if (value > 255)
				m_tempImage[i][j] = 255.;
			else if (value < 0)
				m_tempImage[i][j] = 0;
			else
				m_tempImage[i][j] = value;
		}
	}
...
// 영역처리 중 버리는 것
double** CMFCApplication1Doc::OnMaskProcess2(unsigned char *Target, double Mask[3][3])
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, m, n;
	double **tempInputImage, **tempOutputImage, S = 0.0;

	tempInputImage = Image2DMen(m_height, m_width);
	tempOutputImage = Image2DMen(m_height, m_width);

	for (i = 0; i < m_height; i++) {
		for (j = 0; j < m_width; j++) {
			//계산할 때 값이 변화하기 때문에 원본 유지를 위해서 
...
void CMFCApplication1Doc::OnBlurring()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, sum = 0;
...
// Sharpening 필터
void CMFCApplication1Doc::OnSharpening()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, sum = 0;
	//double SharpMask[3][3] = { {-1,-1,-1},{-1,9,-1},{-1,-1,-1} };
	double SharpMask[3][3] = {{0,-1,0},{-1,5,-1},{0,-1,-1}};
...
// 가우시안 스무딩 필터
void CMFCApplication1Doc::OnGaussianSmooth()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, sum = 0;
	double Gaussian[3][3] = { {1./16., 1./8.,1./16.},{1./8.,1./4.,1./8},{1./16.,1./8.,1./16.} };

	m_Re_height = m_height;
	m_Re_width = m_width;
	m_Re_size = m_Re_height * m_Re_width;
	m_OutputImage = new unsigned char[m_Re_size];
	m_tempImage = OnMaskProcess(m_InputImage, Gaussian);
...
void CMFCApplication1Doc::OnHPF()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j;
...
void CMFCApplication1Doc::OnLPF()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, alpha;
...
// 비트평면분할
void CMFCApplication1Doc::OnBitplane()
{
	// TODO: 여기에 구현 코드 추가.
	int i;
	m_Re_height = m_height;
	...
	for (int i = 0; i < 8; i++) {
		m_OutputImages[i] = new unsigned char[m_Re_size];
		...
		for (int k = 0; k < m_size; k++) {
			if ((m_InputImage[k] & key) == 0)
				m_OutputImages[i][k] = 0;
			else
				m_OutputImages[i][k] = 255;
		}
	}
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			if (dlg.m_Input1 == 1 && dlg.m_Input2 == 245) {
				...
				else if(m_InputImage[i] >= dlg.m_Input2)
					m_OutputImage[i] = (unsigned char)m_InputImage[i] * 0.5;
			}
		}
	}
