...
ON_COMMAND(ID_Blurring, &CMFCApplication1View::OnBlurring)
ON_COMMAND(ID_Sharp, &CMFCApplication1View::OnSharp)
ON_COMMAND(ID_Gaussian_Smooth, &CMFCApplication1View::OnGaussianSmooth)
ON_COMMAND(ID_HPF, &CMFCApplication1View::OnHpf)
ON_COMMAND(ID_LPF, &CMFCApplication1View::OnLpf)
...
void CMFCApplication1View::OnDraw(CDC* pDC)
{
	...
	//입력영상, 히스토그램, 히스토그램 평활화, 히스토그램 평활화 히스토그램 표현
	//불러온 영상을 출력하는 함수
	/*for (i = 0; i < pDoc->m_height; i++) {
		for (j = 0; j < pDoc->m_width; j++) {
			R = G = B = pDoc->m_InputImage[i * pDoc->m_width + j];
	...
	/*for (int k = 0; k < 8; k++) {
		for (i = 0; i < pDoc->m_Re_height; i++) {
			for (j = 0; j < pDoc->m_Re_width; j++) {
				R = pDoc->m_OutputImages[k][i * pDoc->m_Re_width + j];
				G = B = R;
				if (k < 4)
					pDC->SetPixel(j + (pDoc->m_width * k + 10 * k) + 5, i + pDoc->m_height + 10, RGB(R, G, B));
				else
					pDC->SetPixel(j + (pDoc->m_width * (k-4) + 10 * (k-4)) + 5, i + pDoc->m_height*2 + 20, RGB(R, G, B));
			}
		}
	}*/
}
...
//영역처리 블러링 필터
void CMFCApplication1View::OnBlurring()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CMFCApplication1Doc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
//Sharpening 필터 구현
void CMFCApplication1View::OnSharp()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CMFCApplication1Doc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
//가우시안 스무딩 필터
void CMFCApplication1View::OnGaussianSmooth()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CMFCApplication1Doc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);

	pDoc->OnGaussianSmooth();
	Invalidate(TRUE);
}

//High Frequency Band-Pass Filter
void CMFCApplication1View::OnHpf()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CMFCApplication1Doc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
void CMFCApplication1View::OnLpf()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CMFCApplication1Doc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
