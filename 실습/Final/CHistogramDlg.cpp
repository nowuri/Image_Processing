...
void CHistogramDlg::SetImage(CDib& dib)
{
	...
	register int i,k;
	for (k = 0; k < 3; k++) {
		float max_value = histo[k][0];
		for (i = 1; i < 256; i++)
		{
			if (histo[k][i] > max_value) max_value = histo[k][i];
		}
		// m_Histogram 배열의 최대값이 100이 되도록 전체 배열의 값을 조절한다.
		for (i = 0; i < 256; i++)
		{
			m_Histogram[k][i] = (int)(histo[k][i] * 100 / max_value);
		}
	}
}
...
void CHistogramDlg::OnPaint()
{
	...
	{
		dc.MoveTo(20 + i, 130);
		dc.LineTo(20 + i, 130 - m_Histogram[0][i]);
	}
	...
	for (i = 0; i < 256; i++)
	{
		...
		dc.SetDCPenColor(RGB(255-i, 0, 0));
		//dc.SetDCPenColor(RGB(255 - i, 0, 0));
	...
	for (i = 0; i < 256; i++) {
		dc.MoveTo(20 + i, 260);
		dc.LineTo(20 + i, 260 - m_Histogram[1][i]);
	}

	for (i = 0; i < 256; i++)
	{
		dc.SelectStockObject(DC_PEN);
		dc.SetDCPenColor(RGB(0, 255 - i, 0));

		dc.MoveTo(20 + i, 270);
		dc.LineTo(20 + i, 285);
	}

	//blue band 히스토그램 박스 출력
	dc.SelectStockObject(DC_PEN);
	dc.SetDCPenColor(RGB(0, 0, 0));
	...
	for (i = 0; i < 256; i++) {
		dc.MoveTo(20 + i, 390);
		dc.LineTo(20 + i, 390 - m_Histogram[2][i]);
	}

	for (i = 0; i < 256; i++) {
		dc.SelectStockObject(DC_PEN);
		dc.SetDCPenColor(RGB(0, 0, 255 - i));
