...
void CMFCApplication3Doc::DibGeometricRotate1(CDib& dib, CDib& dib1)
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, CenterH, CenterW, newH, newW;
	ImageRotate dlg;
	...
	int h2 = h*cos(90-Radian) + w*cos(Radian);
	int w2 = h*cos(Radian) + w*cos(90-Radian);

	CenterH = h/2;
	CenterW = w/2;

	if (dib.GetBitCount() == 8) {
		dib1.CreateGrayImage(h2, w2);
...
void CMFCApplication3Doc::OnBigProcess(CDib& dib, CDib& dib1)
{
	// TODO: 여기에 구현 코드 추가.
	CFinal2 dlg;
	int w = dib.GetWidth();
	int h = dib.GetHeight();
	if (dlg.DoModal() == IDOK) {
		float size = dlg.m_InputSize;
		int newW = w * size;
		int newH = h * size;
	
		if (dlg.m_ComboSelect==0) {
			DibResizeNearest(dib, newW, newH);
		}
		else if(dlg.m_ComboSelect == 1) {
			DibResizeBilinear(dib, newW, newH);
		}
		else if (dlg.m_ComboSelect == 2) {
			DibResizeCubic(dib, newW, newH);
		}
	}
...
void CMFCApplication3Doc::OnFinal3(CDib& dib, CDib& dib1)
{
	// TODO: 여기에 구현 코드 추가.
	register int i, j, S, n, m;
	int size = 3;
...
void CMFCApplication3Doc::OnStyleWarping(CDib& dib, CDib& dib1)
{
	// TODO: 여기에 구현 코드 추가.
	CHWDialog dlg;
	...
	if (dlg.DoModal() == IDOK) {
		float waveAmplitude = dlg.m_NewHeight;
		float numWaves = dlg.m_NewWidth;
		float r = dlg.m_R;
		...
		float yToPhase = 2.0 * PI * numWaves / (float) h;
		...
		if (dib.GetBitCount() == 8) {
			...
			for (j = 0; j < h; j++) {
				for (i = 0; i < w; i++) {
					int newX, newY;
					newY = int(j + waveAmplitude * sin(i * yToPhase));
					newX = i;
			...
			for (j = 0; j < h; j++) {
				for (i = 0; i < w; i++) {
					if (sqrt((j-w/2)*(j-w/2) + (i-h/2)*(i-h/2))< r) {
						int newX, newY;
						newY = int(j + waveAmplitude * sin(i * yToPhase));
						newX = i;
						...
						ptr1[j][i].r = ptr[j][i].r;
						ptr1[j][i].b = ptr[j][i].b;
						ptr1[j][i].g = ptr[j][i].g;
					}
				}
			}
			//AfxMessageBox(n + " " + n2);
