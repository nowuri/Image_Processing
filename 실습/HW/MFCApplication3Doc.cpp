...
// Face 영역 검출하기
void CMFCApplication3Doc::OnFaceExtraction(CDib& dib)
{
	// TODO: 여기에 구현 코드 추가.
	...
	if (dib.GetBitCount() == 8) {
		...
		for (j = 0; j < h; j++) {
			for (i = 0; i < w; i++) {
				...
				int cb =(int)(128 - 0.16874 * r - 0.33126 * g + 0.5 * b);
				int cr = (int)(128+0.5 * r - 0.41869 * g - 0.0813 * b);
				...
				if (y>40 && y<=230 &&cb >= 10 && cb <= 120 && cr >= 140 && cr <= 170) {
					ptrrgb[j][i].r = r;
					ptrrgb[j][i].b = b;
					ptrrgb[j][i].g = g;
				}
				...
				//방법 4: rgb 이용
				/*if (r>80 && r <= 240 && g > 40 && g <= 200 && b >= 20 && b <= 180) {
					ptrrgb[j][i].r = r;
					ptrrgb[j][i].b = b;
					ptrrgb[j][i].g = g;
				}
